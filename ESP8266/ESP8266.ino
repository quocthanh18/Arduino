...
const char * API = "LYHQ4LEB771FAA5R";
...
void handleNormalNoti(const int value[], bool watered, bool servo_state, const char* email){
...
void displayOLED(int sensors[3]){
  for (int i = 0; i < 3; i++) {
...
void websiteHandler(const int sensors[], bool watered, bool servo_state){
  ...
  mqttClient.publish("21127428_21127273/Watered", watered_);
  const char* servo_ = servo_state ? "Yes" : "No";
  mqttClient.publish("21127428_21127273/Servo", servo_);
} 
...
void loop() {
  ...
  int sensors[3];
  bool watered = false, servo_state = false;
  if (sensor.available()) {
    char json[200];
    int index = 0;
  ...
  // Notify user
  handleNormalNoti(sensors, watered, servo_state, "thanhwtf95@gmail.com");
  handleEmergencyNoti(sensors, "thanhwtf95@gmail.com");

  //Send info to website
  websiteHandler(sensors, watered, servo_state);

  //Send info to cloud
  cloudHandler(sensors);

  //Receive instructions from website
